...
class FileAttachment < ActiveRecord::Base
  has_attachment :path_prefix => 'vendor/extensions/assets/test/files', :processor => :rmagick
  validates_as_attachment
end

class ImageFileAttachment < FileAttachment
  has_attachment :path_prefix => 'vendor/extensions/assets/test/files',
...
class ImageWithThumbsClassFileAttachment < FileAttachment
  ...
  has_attachment :file_system_path => 'vendor/extensions/assets/test/files',
    :thumbnails => { :thumb => [50, 50] }, :resize_to => [55,55],
    :thumbnail_class => 'ImageThumbnail'
end

class ImageThumbnail < FileAttachment
  has_attachment :path_prefix => 'vendor/extensions/assets/test/files/thumbnails'
end
